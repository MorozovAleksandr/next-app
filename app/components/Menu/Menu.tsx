...
const Menu = async ({firstCategory = 1}: { firstCategory: LevelCategory }): Promise<React.JSX.Element> => {
    ...
    const buildThirdLevel = (pages: IPageItem[], route: string) => pages.map(p => (
        <a key={p.alias} href={`/${route}/${p.alias}`}
           className={classnames(style.thirdLevel, {[style.thirdLevelActive]: false})}>
            {p.category}
        </a>
    ));
